...
block([
    ...
    regCommand('insertfile', function(file) {
        ...
        if (_.util.bool.isStr(val)) {
            ...
            if (_.util.bool.isStr(name)) {
                var html = '<a href="' + val + '" target="_blank" title="click to download" class="bc se-attachment">' + name + '</a><br />';
            } else {
                var html = 'Attachment : <a href="' + val + '" target="_blank" title="click to download" class="bc se-attachment">' + val + '</a><br />';
            }
    ...
    regCreater('insertfile', function() {
        ...
        html += '<span class="bc se-title">Insert Files</span>';
        html += '<div class="bc se-aaa">';
        html += '<label>Alias</label><input type="text" class="bc se-input" placeholder="Enter Attachment Anchor Alias" />';
        html += '</div>';
        html += '<div class="bc se-url">';
        html += '<label>File URL</label><input type="text" class="bc se-input" placeholder="Enter URL" />';
        html += '</div>';
        html += '<input type="file" class="bc se-files" value="" hidden="" />';
    ...
    regDialog('insertfile', function(btn) {
        ...
        var n_input = _.query('.bc.se-aaa .bc.se-input', dialog)[0],
            v_input = _.query('.bc.se-url .bc.se-input', dialog)[0];
    ...
    regDialog('uploadfile', function(btn) {
        var dialog = _.dom.closest(btn, 'dialog');
        var input = _.query('.bc.se-files', dialog)[0];
        var that = this;
