...
class Admin_LinksViewModel extends \PM\_2\BaseCRUDViewModel {
	public static
	$model = 'Pages\Models\LinkModel',
	$columns = [
		[
			'field_name'	=>	'__index',
			'display_name'	=>	'序号',
	...
	protected function buildTableRows($basedir, $items = []){
		...
		foreach($items as $index=>$link){
			$stage_url = $basedir.'/'.$link->id;
			$rows[] = [
				'__index'	=>	[$index],
				'name'		=>	[$link->name, $stage_url, false],
				'type'		=>	[self::$types[$link->type]],
				'sort'		=>	[$link->sort],
				'alt'		=>	[$link->alt],
				'state'		=>	[$link->state?'正常':'隐藏'],
				'__ops'		=>	['<a href="'. $link->getLinkURL($frontdir).'" target="_blank">访问</a> | <a data-onclick="delete" data-submit-href="'.$stage_url.'" href="javascript:;">移除</a>']
			];
		}
		return $rows;
	}
