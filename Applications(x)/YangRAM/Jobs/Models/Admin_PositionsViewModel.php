...
class Admin_PositionsViewModel extends \PM\_2\BaseTableViewModel {
	public static
	$columns = [
		[
			'field_name'	=>	'__index',
	...
	public function analysis($admininfo){
		...
		foreach($positions as $index=>$position){
			$stage_url = $stagedir.'/Position/'.$position->ID;
			$rows[] = [
				'__index'		=>	[$index + 1],
				'title'		=>	[$position->POSINAME, $stage_url, false],
				'crttime'	=>	[$position->SK_CTIME],
				'modtime'	=>	[$position->SK_MTIME],
				'pubtime'	=>	[$position->PUBTIME],
				'__count'	=>	[0],
				'__ops'		=>	['<a href="'.$stage_url.'">编辑</a> | <a data-onclick="delete" data-submit-href="/applications/cloudtables/rows/'.$position->ID.'" href="javascript:;">移除</a>']
			];
		}
		self::$creater['url'] = $stagedir.'/Position/';
