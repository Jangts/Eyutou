<?php
namespace Services\Task\Controllers;
use Status;
 ...
 *  
 */
class WorkerController extends Process {
    const LOG_PATH = '/tmp/';
    ...
    private function doServer() {
        ...
        foreach ($sockets as $socket) {
            ...
            if ($socket == $this->master) {
                ...
                // 创建,绑定,监听后accept函数将会接受socket要来的连接,一旦有一个连接成功,将会返回一个新的socket资源用以交互,如果是一个多个连接的队列,只会处理第一个,如果没有连接的话,进程将会被阻塞,直到连接上.如果用set_socket_blocking或socket_set_notang.block()设置了阻塞,会返回false;返回资源后,将会持续等待连接。
                if (false === $client) {
                    $this->error([
                        'err_accept',
                        $err_code = socket_last_error(),
