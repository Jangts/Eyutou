...
class OperationScript {
            ...
            if($count>1){
                for($i = 1; $i < $count; $i++){
                    ...
                    if(isset($exp[2])){
                        ...
                        new Status(1414, 'Syntactic Error :'.$exps[$i], true);
                    }
                }
            }
    ...
    private function classify(){
        ...
        foreach($this->midast as $exp){
            switch($exp['type']){
                ...
                case 'const method':
                if(in_array($exp['name'], $this->constnames)){
                    new Status(1461, 'Connot Redeclare Const '.$exp['name'], true);
                }else{
                    $this->constnames[] = $exp['name'];
                    $exp['type'] = 'method';
                    $exps[0][$exp['name']] = $exp;
    ...
    private function generator($RUNTIME, $outfile, $minfile, $lang){
        ...
        Filesys::writeContent($outfile, $code);
        Filesys::writeContent($minfile, $mini);
        if(_USE_DEBUG_MODE_){
			$this->code = $code;
		}else{
			$this->code = $mini;
