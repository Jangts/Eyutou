...
class TimeModel implements \DataModel {
	...
	public function __construct($lang){
		if(is_file(LIBX_PATH.'localtimes/'.$lang.'.json')&&($data = json_decode(LIBX_PATH.'localtimes/'.$lang.'.json', true))){
			$this->data = $data;
		}else{
			$la = substr($lang, 0, 2);
			$files = glob(LIBX_PATH.'localtimes/'.$la.'*.json');
			if(isset($files[0])){
				if(is_file($files[0])&&($data = json_decode($files[0], true))){
					$this->damodelPropertiesta = $data;
				}
			}
		}
	}
