...
/**
 * @class Tangram\CTRLR\rdb_drivers\_abstract
 ...
 * @abstract 抽象类
**/
abstract class _abstract extends PDO {
    ...
    public static function parseDsn(array $options){
        $sp = new Status(1416.1, 'SQL Driver Error', 'SQL Driver File Must Be Tampered, Please Check Your Driver Files');
        $sp->log();
    }

    protected $user, $dbname, $locked__key = NULL;

    public function lock(){
        if($this->locked__key === NULL){
            return $this->locked__key = uniqid();
        }
        return false;
    }
    ...
    public function unlock($__key){
        if($this->locked__key === $__key){
            $this->locked__key = NULL;
            return true;
        }
        return false;
    }
    ...
    public function query($statement){
        try {
            ...
            $sp = new Status(1416.3, 'SQL Query Error', $statement);
            $sp->write($e->getMessage());
            if(_USE_DEBUG_MODE_){
                return $sp->respond();
            }
            $sp->log();
    ...
    public function exec($statement){
        try {
            ...
            $sp = new Status(1416.4, 'SQL Exec Error', $statement);
            $sp->write($e->getMessage());
            if(_USE_DEBUG_MODE_){
                return $sp->respond();
            }
