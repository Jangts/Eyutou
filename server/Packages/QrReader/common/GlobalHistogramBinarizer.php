...
//@Override
    public function getBlackRow($y, $row=null) {
        ...
        $localdictLuminances = $this->source->offsetGet($y, $this->luminances);
        $localdictBuckets = $this->buckets;
        for ($x = 0; $x < $width; $x++) {
            $pixel = $localdictLuminances[$x] & 0xff;
            $localdictBuckets[$pixel >> self::$LUMINANCE_SHIFT]++;
        }
        $blackPoint = $this->estimateBlackPoint($localdictBuckets);

        $left = $localdictLuminances[0] & 0xff;
        $center = $localdictLuminances[1] & 0xff;
        for ($x = 1; $x < $width - 1; $x++) {
            $right = $localdictLuminances[$x + 1] & 0xff;
...
// more robust on the blackbox tests than sampling a diagonal as we used to do.
        ...
        for ($y = 1; $y < 5; $y++) {
            ...
            $localdictLuminances = $source->offsetGet($row, $this->luminances);
            $right = intval(($width * 4) / 5);
            for ($x = intval($width / 5); $x < $right; $x++) {
                $pixel = intval32bits($localdictLuminances[intval($x)] & 0xff);
                $localdictBuckets[intval32bits($pixel >> self::$LUMINANCE_SHIFT)]++;
            }
        }
        $blackPoint = $this->estimateBlackPoint($localdictBuckets);
...
// "fail quickly" which is necessary for continuous scanning.
        $localdictLuminances = $source->getMatrix();
        for ($y = 0; $y < $height; $y++) {
            $offset = $y * $width;
            for ($x = 0; $x< $width; $x++) {
                $pixel = intval($localdictLuminances[$offset + $x] & 0xff);
                if ($pixel < $blackPoint) {
                    $matrix->set($x, $y);
                }
            }
