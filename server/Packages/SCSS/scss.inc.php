...
class scssc {
	...
	public function compile($code, $name = null)
	{
		...
		$this->extendedPropertiess      = array();
		$this->extendedPropertiessMap   = array();
	...
	protected function pushExtends($target, $origin) {
		...
		$i = count($this->extendedPropertiess);
		$this->extendedPropertiess[] = array($target, $origin);

		foreach ($target as $part) {
			if (isset($this->extendedPropertiessMap[$part])) {
				$this->extendedPropertiessMap[$part][] = $i;
			} else {
				$this->extendedPropertiessMap[$part] = array($i);
			}
		}
	}
	...
	protected function matchExtendsSingle($single, &$outOrigin) {
		...
		foreach ($single as $part) {
			...
			if (isset($this->extendedPropertiessMap[$part])) {
				foreach ($this->extendedPropertiessMap[$part] as $idx) {
					$counts[$idx] =
		...
		foreach ($counts as $idx => $count) {
			list($target, $origin) = $this->extendedPropertiess[$idx];
	...
	protected function flattenSelectors($block, $parentKey = null) {
		if ($block->selectors) {
			...
			foreach ($block->selectors as $s) {
				...
				// check extends
				if (!empty($this->extendedPropertiessMap)) {
